...
const AddRecord = () => {
  ...
  const getNewTimeStamp = () => {
    ...
    setAudioCropTimeStamp(prevList => [...prevList, currentStopWatchTimeStamp]);
  };
  ...
  const stopAudioRecording = () => {
    ...
    console.log(audioCropTimeStamp)
    resetStopwatch();
  ...
  const sendAudioToServer = async (wavFileName) => {
    ...
    const blob = new Blob([audioCropTimeStamp.join('\n')], { type: 'text/plain' });
    const formData = new FormData(aud);
    formData.append('audioFile', audioBlob, `${patientId}.wav`);
    formData.append('timeStampTextFile', blob, `${getWavFileName()}`.wav);
    try {
  ...
  const downloadRecording = () => {
    ...
    for (var i = 1; i < audioCropTimeStamp.length; i++) {
      paired.push(audioCropTimeStamp[i-1] + " " + audioCropTimeStamp[i])
    }
